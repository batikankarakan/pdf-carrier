...
from .text_search_encryption import (
    extract_full_text,
...
def encrypt_pdf_selective(
    ...
    """
    Encrypt only selected areas of a PDF

    Process:
    1. Extract all text with positions
    2. Filter text inside vs outside selected areas
    3. Encrypt text inside selected areas
    4. Create new PDF with encrypted areas
    5. Store original area content in key file for decryption

    Args:
        pdf_bytes: PDF file content
        areas: List of area dictionaries with page_number, x, y, width, height
        original_filename: Name of the original file
        algorithms: Optional list of algorithm names to use (must be exactly 2)
    ...
    Returns:
        Tuple of (encrypted_pdf_bytes, key_file_bytes)
    """
    print(f"[ENCRYPT-SELECTIVE] Starting selective encryption...")
    print(f"[ENCRYPT-SELECTIVE] Areas to encrypt: {len(areas)}")

    # Step 1: Get page info FIRST (needed for coordinate transformation)
    page_info = get_pdf_page_info(pdf_bytes)
    print(f"[ENCRYPT-SELECTIVE] PDF has {page_info['page_count']} pages")
    for pnum, pinfo in page_info['pages'].items():
        print(f"[ENCRYPT-SELECTIVE] Page {pnum}: width={pinfo['width']:.1f}, height={pinfo['height']:.1f}")

    # Convert area dicts to AreaSelection objects, transforming canvas coords to PDF coords
    area_selections = [AreaSelection.from_dict(a, page_info) for a in areas]

    # Debug: print transformed coordinates
    for i, area in enumerate(area_selections):
        print(f"[ENCRYPT-SELECTIVE] Area {i} (PDF coords): page={area.page_number}, x={area.x:.1f}, y={area.y:.1f}, w={area.width:.1f}, h={area.height:.1f}")

    # Step 2: Extract text with positions
    print(f"[ENCRYPT-SELECTIVE] Extracting text with positions...")
    text_elements = extract_text_with_positions(pdf_bytes)
    print(f"[ENCRYPT-SELECTIVE] Found {len(text_elements)} text elements")

    # Step 3: Filter text inside vs outside areas
    text_in_areas, text_outside_areas = filter_text_in_areas(text_elements, area_selections)
    print(f"[ENCRYPT-SELECTIVE] Text in areas: {len(text_in_areas)}, outside: {len(text_outside_areas)}")
    ...
    if algorithms:
        ...
        if not all(algo in AVAILABLE_ALGORITHMS for algo in algorithms):
            raise ValueError(f"Invalid algorithms. Available: {AVAILABLE_ALGORITHMS}")
        selected_algorithms = algorithms
        print(f"[ENCRYPT-SELECTIVE] Using user-selected algorithms: {selected_algorithms}")
    else:
        selected_algorithms = select_random_algorithms(2)
        print(f"[ENCRYPT-SELECTIVE] Randomly selected algorithms: {selected_algorithms}")

    # Step 6: Group text by area and encrypt each area's content
    area_content = group_text_by_area(text_in_areas, area_selections)

    # Encrypt each area's content
    encrypted_area_content = {}
    encrypted_display_text = {}
    layer_data = {}
    symmetric_keys = {}

    for area_id, content_data in area_content.items():
        # Even if no text detected, we still encrypt a placeholder
        # This ensures the area is always covered with encrypted-looking content
        text_to_encrypt = content_data['text'] if content_data['text'] else "[REDACTED CONTENT]"

        # Encrypt the area's text content
        content_bytes = text_to_encrypt.encode('utf-8')
        current_data = content_bytes

        area_layer_data = {}
        area_symmetric_keys = {}

        # Apply each selected algorithm
        for i, algorithm in enumerate(selected_algorithms):
            layer_num = i + 1
            ...
            if algorithm == "AES-256-GCM":
                key = AESGCMCipher.generate_key()
                ciphertext, iv = AESGCMCipher.encrypt(current_data, key)
                area_layer_data[f"layer{layer_num}_iv"] = base64.b64encode(iv).decode('utf-8')
                area_symmetric_keys[f"layer{layer_num}_key"] = key
                current_data = ciphertext
                ...
                key = ChaCha20Cipher.generate_key()
                ciphertext, nonce = ChaCha20Cipher.encrypt(current_data, key)
                area_layer_data[f"layer{layer_num}_nonce"] = base64.b64encode(nonce).decode('utf-8')
                area_symmetric_keys[f"layer{layer_num}_key"] = key
                current_data = ciphertext
        ...
        # Store encrypted data and keys for this area
        encrypted_area_content[area_id] = {
            'encrypted_data': base64.b64encode(current_data).decode('utf-8'),
            'original_text': text_to_encrypt,
            'layer_data': area_layer_data,
            'elements': content_data['elements']
        }

        # Create display text (truncated base64)
        display_b64 = base64.b64encode(current_data).decode('utf-8')
        encrypted_display_text[area_id] = display_b64

        # Store keys for this area
        layer_data[area_id] = area_layer_data
        symmetric_keys[area_id] = area_symmetric_keys

    print(f"[ENCRYPT-SELECTIVE] Encrypted {len(encrypted_area_content)} areas")

    # Step 7: Create PDF with encrypted areas (overlay on original)
    encrypted_pdf = create_pdf_with_selective_encryption(
        pdf_bytes,
        encrypted_display_text,
        area_selections,
        page_info
    )
    print(f"[ENCRYPT-SELECTIVE] Created encrypted PDF: {len(encrypted_pdf)} bytes")

    # Step 8: Create combined keys blob for RSA encryption
    all_keys = b''
    for area_id in encrypted_area_content:
        if area_id in symmetric_keys:
            for i in range(len(selected_algorithms)):
                key = symmetric_keys[area_id].get(f"layer{i+1}_key", b'')
                all_keys += key
    ...
    # Step 9: Generate HMAC for integrity
    hmac_key = HMACGenerator.generate_key()
    hmac_tag = HMACGenerator.compute(encrypted_pdf, hmac_key)

    # Step 10: Create metadata
    metadata = {
        "version": "5.0",  # New version for selective encryption
        "encryption_type": "selective",
        "algorithms": selected_algorithms,
        "encrypted_symmetric_keys": base64.b64encode(encrypted_symmetric_keys).decode('utf-8') if encrypted_symmetric_keys else '',
        "areas": [a.to_dict() for a in area_selections],
        "encrypted_area_content": encrypted_area_content,
        "hmac_key": base64.b64encode(hmac_key).decode('utf-8'),
        "hmac": base64.b64encode(hmac_tag).decode('utf-8'),
        "timestamp": datetime.utcnow().isoformat() + "Z",
        "original_filename": original_filename,
    ...
    print(f"[ENCRYPT-SELECTIVE] Encryption complete!")
    return encrypted_pdf, key_file_bytes


def encrypt_pdf_text_search(
    pdf_bytes: bytes,
    ...
    """
    Encrypt specific text patterns/keywords in a PDF

    Process:
    1. Find all occurrences of search terms in PDF
    2. Encrypt each unique term
    3. Create annotated PDF with encryption info
    4. Store mappings in key file for decryption

    Args:
        pdf_bytes: PDF file content
        search_terms: List of terms to search for and encrypt
        original_filename: Name of the original file
    ...
    Returns:
        Tuple of (encrypted_pdf_bytes, key_file_bytes)
    """
    print(f"[ENCRYPT-TEXT-SEARCH] Starting text search encryption...")
    print(f"[ENCRYPT-TEXT-SEARCH] Search terms: {search_terms}")
    print(f"[ENCRYPT-TEXT-SEARCH] Case sensitive: {case_sensitive}")

    # Step 1: Find all occurrences
    occurrences = find_text_occurrences(pdf_bytes, search_terms, case_sensitive)

    total_occurrences = sum(len(v) for v in occurrences.values())
    print(f"[ENCRYPT-TEXT-SEARCH] Found {total_occurrences} total occurrences")

    for term, matches in occurrences.items():
        print(f"[ENCRYPT-TEXT-SEARCH] - '{term}': {len(matches)} occurrences")
    ...
    if algorithms:
        ...
        if not all(algo in AVAILABLE_ALGORITHMS for algo in algorithms):
            raise ValueError(f"Invalid algorithms. Available: {AVAILABLE_ALGORITHMS}")
        selected_algorithms = algorithms
        print(f"[ENCRYPT-TEXT-SEARCH] Using user-selected algorithms: {selected_algorithms}")
    else:
        selected_algorithms = select_random_algorithms(2)
        print(f"[ENCRYPT-TEXT-SEARCH] Randomly selected algorithms: {selected_algorithms}")

    # Step 4: Encrypt each unique search term
    encrypted_terms = {}
    encrypted_display = {}
    layer_data = {}
    symmetric_keys = {}
    ...
    for term in search_terms:
        # Encrypt the term
        content_bytes = term.encode('utf-8')
        current_data = content_bytes
        ...
        for i, algorithm in enumerate(selected_algorithms):
            ...
            if algorithm == "AES-256-GCM":
                key = AESGCMCipher.generate_key()
                ciphertext, iv = AESGCMCipher.encrypt(current_data, key)
                term_layer_data[f"layer{layer_num}_iv"] = base64.b64encode(iv).decode('utf-8')
                term_symmetric_keys[f"layer{layer_num}_key"] = key
                current_data = ciphertext
                ...
                key = ChaCha20Cipher.generate_key()
                ciphertext, nonce = ChaCha20Cipher.encrypt(current_data, key)
                term_layer_data[f"layer{layer_num}_nonce"] = base64.b64encode(nonce).decode('utf-8')
                term_symmetric_keys[f"layer{layer_num}_key"] = key
                current_data = ciphertext
        ...
        encrypted_terms[term] = {
            'encrypted_data': base64.b64encode(current_data).decode('utf-8'),
            'original_text': term,
            'layer_data': term_layer_data,
            'occurrences': len(occurrences.get(term, []))
        }

        # Create display text (short base64 preview)
        display_b64 = base64.b64encode(current_data).decode('utf-8')[:20]
        encrypted_display[term] = display_b64

        # Store keys for this term
        layer_data[term] = term_layer_data
        symmetric_keys[term] = term_symmetric_keys

    print(f"[ENCRYPT-TEXT-SEARCH] Encrypted {len(encrypted_terms)} unique terms")

    # Step 5: Create redacted PDF with encrypted text replacements
    encrypted_pdf = create_redacted_pdf(pdf_bytes, search_terms, encrypted_display, case_sensitive)
    print(f"[ENCRYPT-TEXT-SEARCH] Created redacted PDF: {len(encrypted_pdf)} bytes")

    # Step 6: Create combined keys blob for RSA encryption
    all_keys = b''
    for term in search_terms:
        if term in symmetric_keys:
            for i in range(len(selected_algorithms)):
                key = symmetric_keys[term].get(f"layer{i+1}_key", b'')
                all_keys += key
    ...
    page_info = get_pdf_page_dimensions(pdf_bytes)

    # Step 9: Encrypt the original PDF for secure storage
    # Generate a dedicated key for encrypting the original PDF using CEM algorithm
    pdf_encryption_key = _key_generator.generate_key(32)  # 256-bit key for AES
    pdf_iv = _key_generator.generate_key(16)  # IV for AES

    # Use AES-CBC to encrypt the original PDF
    ...
    print(f"[ENCRYPT-TEXT-SEARCH] Original PDF encrypted: {len(encrypted_original_pdf)} bytes")

    # Step 10: Create metadata
    metadata = {
        "version": "6.1",  # Updated version for encrypted original PDF
        "encryption_type": "text_search",
        "algorithms": selected_algorithms,
        "encrypted_symmetric_keys": base64.b64encode(encrypted_symmetric_keys).decode('utf-8') if encrypted_symmetric_keys else '',
        "search_terms": search_terms,
        "case_sensitive": case_sensitive,
        "encrypted_terms": encrypted_terms,
        "occurrences": {term: len(matches) for term, matches in occurrences.items()},
        "hmac_key": base64.b64encode(hmac_key).decode('utf-8'),
        "hmac": base64.b64encode(hmac_tag).decode('utf-8'),
        "timestamp": datetime.utcnow().isoformat() + "Z",
        "original_filename": original_filename,
    ...
    print(f"[ENCRYPT-TEXT-SEARCH] Encryption complete!")
    return encrypted_pdf, key_file_bytes
...
__all__ = [
    'encrypt_pdf',
    'encrypt_pdf_selective',
    'encrypt_pdf_text_search',
